...
int _open_PLX9050(unsigned int *BASEA, unsigned int *BASEB, int *pci_handle,
                  unsigned int *mmap_io_ptr, int *interrupt_line)
{
  ...
  *pci_handle = pci_attach(flags);
  ...
  if (temp != PCI_SUCCESS) {
    perror("Cannot read from configuration space of the GC314 PCI/FS "
           "digital receiver card");
    return -1;
  }
  ...
  BASE0 = mmap_device_memory(0, 256,
                              PROT_EXEC|PROT_READ|PROT_WRITE|PROT_NOCACHE, 0,
                              pci_reg.Base_Address_Regs[0]);
  BASE1 = mmap_device_memory(0, 1048576,
                              PROT_EXEC|PROT_READ|PROT_WRITE|PROT_NOCACHE, 0,
  ...
  *interrupt_line = pci_reg.Interrupt_Line;
  ...
  while (uio_device_list) {
    ...
    printf("%d: %d : name=%s, version=%s, events=%d\n",
           uio_device_list->uio_num, strcmp(uio_device_list->name,MODULE_NAME),
           uio_device_list->name,uio_device_list->version,
           uio_device_list->event_count);
  ...
  BASE0 = page_offset+mmap(NULL, gps_device->maps[0].size,
  ...
  BASE1 = page_offset+mmap(NULL, gps_device->maps[1].size,
                            PROT_READ|PROT_WRITE ,MAP_SHARED,fd,
                            1*getpagesize());
