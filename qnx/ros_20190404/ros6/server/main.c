...
pthread_mutex_t ctrlprog_list_lock,ros_state_lock,coord_lock,exit_lock,clr_lock;
...
char *ctrlprog_list_lock_buff=NULL;
...
void graceful_cleanup(int signum)
{
  ...
  if (thread_list!=NULL) {
    while (thread_list!=NULL) {
      ...
      //logger(&ctrlprog_list_lock_buff,PRE,LOCK,"main_graceful_cleanup",-1,-1,0);
      pthread_mutex_lock(&ctrlprog_list_lock);
      //logger(&ctrlprog_list_lock_buff,POST,LOCK,"main_graceful_cleanup",-1,-1,0);
      ...
      //logger(&ctrlprog_list_lock_buff,PRE,UNLOCK,"main_graceful_cleanup",-1,-1,0);
      pthread_mutex_unlock(&ctrlprog_list_lock);
      //logger(&ctrlprog_list_lock_buff,POST,UNLOCK,"main_graceful_cleanup",-1,-1,0);
      t++;
    }
  }
  if (verbose>0)
  ...
  close(gpssock);   
  pthread_mutex_destroy(&ctrlprog_list_lock);
...
/* end DIO */
  ...
  ctrlprog_list_lock_buff = realloc(NULL, 1);
  strcpy(ctrlprog_list_lock_buff, "");
 ...
 */
  ...
  pthread_mutex_init(&ctrlprog_list_lock, NULL);
 ...
 */
     ...
     while(num_threads<Max_Control_THREADS){
       ...
       gettimeofday(&current_time,NULL);
        //logger(&ctrlprog_list_lock_buff,PRE,LOCK,"main_client_connect",-1,-1,0);
       pthread_mutex_lock(&ctrlprog_list_lock);
        //logger(&ctrlprog_list_lock_buff,POST,LOCK,"main_client_connect",-1,-1,0);
       ...
       while(thread_list!=NULL){
        ...
        //logger(&ctrlprog_list_lock_buff,PRE,UNLOCK,"main_client_connect",-1,-1,0);
       pthread_mutex_unlock(&ctrlprog_list_lock);
        //logger(&ctrlprog_list_lock_buff,POST,UNLOCK,"main_client_connect",-1,-1,0);
