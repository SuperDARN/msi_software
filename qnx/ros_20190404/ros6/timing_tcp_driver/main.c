...
// jtk 9/2015, hack in code for generating delayed tr pulses on tx line
// for controlling target
// set ENABLE_TARGET_GATE to 1 to enable, then recompile
// set TARGET_DELAY_US to tr delay in microseconds
// (so, TARGET_DELAY_US / C / 2 / 1e6 is range?)
#define ENABLE_TARGET_GATE 0
// time delay between TR and TX pulses in microseconds
#define TARGET_DELAY_US 6000 

int sock,msgsock;
int verbose = 2;
int configured = 1; // SGS 20190404 added
int writingFIFO=0, dma_count=0, under_flag=0,empty_flag=0;
int max_seq_count, xfercount, totransfer, IRQ, intid;
    ...
    if (writingFIFO) {
      ...
      if (xfercount < max_seq_count) {
        ...
        if (xfercount<max_seq_count) {
          ...
          DMAxfer(BASE0_dio, physical_addr[0]+4*xfercount, 0x14,
                  4*totransfer);
...
// PCI-7300A variables
  ...
  //int pseq[7]={0, 3, 4, 6}, scope_sync[16384], TR[16384], TX[16384];
  //int TX_array[16384], trigger[16384], FIFOlevel[16384];
  //int tau=2400, tperiod=1, tlength=300, time_array[10], intt=200;
  //int loopcount=0, fifocnt=0;
  int delay_count;
  ...
  temp = _open_PLX9080(&BASE0_dio, &io_BASE1_dio, &BASE1_phys_dio,
                      &pci_handle_dio, &mmap_io_dio, &IRQ_dio, 1);
  IRQ = IRQ_dio;
  ...
  if (temp == -1) {
    ...
    configured = 0;
  } else fprintf(stderr, "PLX9080 configuration successful");
  ...
  temp = _create_DMA_buff(&virtual_addr[0], &physical_addr[0],
                          4*MAX_TIME_SEQ_LEN);
  master_buf = (unsigned int*)virtual_addr[0];
  if (temp == -1) fprintf(stderr, "ERROR MAKING DMA BUFFERS!\n");
#else
  ...
  if (configured) {
    ...
    out32(mmap_io_ptr_dio+0x04, 0x00000001);  //output is 32bits, timer1 is used
                                              //  pattern gen disabled, etc
    out32(mmap_io_ptr_dio+0x04, 0x00000601);  //clear fifo, and clear under-run
                                              //  status bit
    // SET UP 8254 TIMER CHIP
    out32(mmap_io_ptr_dio+0x2c, 0x00000000); //select timer0
    out32(mmap_io_ptr_dio+0x2c, 0x00000006); //select mode3, squarewave genertr
    ...
    out32(mmap_io_ptr_dio+0x1c, 0x00000000); //set triggs to rising edg polarity 
    temp=in32(mmap_io_ptr_dio+0x04);
    out32(mmap_io_ptr_dio+0x04, 0x00000041); // wait for trigg terminations off
    temp=256;
    out32(mmap_io_ptr_dio+0x18, ((temp<<16) | temp) ); //set FIFO thrshlds to 0 
    out32(mmap_io_ptr_dio+0x18, ((temp<<16) | temp) ); //set FIFO thrshlds to 0 
          ...
          switch (datacode) {
            ...
            case TIMING_REGISTER_SEQ:
              ...
              if (verbose > 1) {
                ...
                printf("Attempting Free on pulseseq :p\n",
                          pulseseqs[r][c][index]); 
              }
            ...
            case TIMING_CtrlProg_READY:
              ...
              if (1==1) {
                if (verbose > 1) {
                  printf("Need to unpack pulseseq\n"); 
                  printf("Pulseseq length: %d\n",pulseseqs[r][c][index]->len); 
                }
                // unpack the timing sequence
                seq_count[r][c]=0;
                step = (int)((double)pulseseqs[r][c][index]->step/
                             (double)STATE_TIME+0.5);

                //If DDS or RX Offset is negative pad the seq_buf iwith the
                // maximum negative offset
                offset_pad = (int)((double)MIN(DDS_OFFSET,RX_OFFSET)/
                                  ((double)STATE_TIME+0.5))-2;
              ...
              if (verbose > 1)
                printf("Timing Card seq length: %d state step: %lf time: %lf\n",
                        seq_count[r][c],STATE_TIME*1E-6,
                        STATE_TIME*1E-6*seq_count[r][c]);
            ...
            case TIMING_PRETRIGGER:
              if (verbose > 1) {
                ...
                printf("Setup Timing Card for next trigger\n");  
                printf("Max Seq length: %d Num clients: %d\n",
              ...
              if (verbose > 1)
                printf("Timing Driver: %d %d\n",new_seq_id,old_seq_id);

              if (new_seq_id != old_seq_id) { 
                if (verbose > -1)
                  printf("Calculating Master sequence %d %d\n",
                ...
                for (i=0;i<numclients;i++) {
                  r = clients[i].radar-1;
                  c = clients[i].channel-1;
                  if (seq_count[r][c]>=max_seq_count)
                      max_seq_count = seq_count[r][c];
                  if (verbose > 1) printf("Max Seq length %d\n",max_seq_count); 
                  counter=0;
                  if (verbose > 1)
                    printf("Merging Client Seq %d into Master Seq %d %d "
                          "length:%d\n", i,r,c,seq_count[r][c]); 
                ...
                rx_trigger=0;
                if (verbose > 1)
                  printf("Fifo stamping Master Seq using FIFOLVL %d\n",
                          FIFOLVL); 
                ...
                for (i=0;i<max_seq_count;i++) {
                  ...
                  // jtk 9/2015, hack in TX line as delayed TR sig for targets
                    if (ENABLE_TARGET_GATE) {
                      temp = TARGET_DELAY_US / STATE_TIME;
                      if (i + temp < MAX_TIME_SEQ_LEN) {
                    ...
                    if (tr_event==0) { 
                      if (verbose > 1)
                        printf("Master TR sample start: %d %x\n",
                                i,master_buf[i]); 
                    ...
                    if (tr_event==1) 
                      if (verbose > 1 )
                        printf("Master TR sample end: %d %x\n",i,master_buf[i]);
                    tr_event=0;
                  }
                  ...
                  if ((master_buf[i] & 0x01)==0x01) {
                    if (scope_event==0) { 
                      if (verbose > 1)
                        printf("Scope Sync sample start: %d %x\n",
                                i,master_buf[i]); 
                      scope_start=i;
                    }
                    scope_event=1;
                    ...
                    if (scope_event==1) 
                      if (verbose > 1)
                        printf("Scope Sync sample end: %d %x\n",
                                i,master_buf[i]); 
                    scope_event=0;
                  }
                }
                ...
                if (scope_start>-1) { 
                  dds_trigger=scope_start+
                            (int)((double)DDS_OFFSET/((double)STATE_TIME+0.5));
                  rx_trigger=scope_start+
                            (int)((double)RX_OFFSET/((double)STATE_TIME+0.5));
                  if (verbose > 1) {
                  ...
                  if (verbose > 1)
                    printf("XXX> Scope Sync not in Master %ld\n",max_seq_count);
                  dds_trigger=0;
                  rx_trigger=0;
                }
              ...
              if (verbose > 1) {
                printf("seq length: %d state step: %lf time: %lf\n",
                        max_seq_count,STATE_TIME*1E-6,
                        (STATE_TIME*1E-6*max_seq_count));
                printf("Max Seq Count:%d\n",max_seq_count);  
                printf("END FIFO Stamps\n");
              }
              ...
              if (new_seq_id < 0) old_seq_id = -10;
              else                old_seq_id = new_seq_id;
              new_seq_id = -1;

              send_data(msgsock, &bad_transmit_times.length,
            ...
            case TIMING_TRIGGER:
              if (verbose > 1) {
                ...
                printf("Read msg struct from tcp socket!\n");  
              }
              ...
              if (verbose > 1) {
                ...
                printf(" Trigger Time: %d %d\n",t1.tv_sec,t1.tv_usec);  
                printf("End Timing Card trigger\n");  
              }
              rval = send_data(msgsock, &msg, sizeof(struct DriverMsg));
              break;
            ...
            case TIMING_GPS_TRIGGER:
              ...
              if (configured) {
                ...
                //out32(mmap_io_ptr_dio+0x04, 0x0141);
                out32(mmap_io_ptr_dio+0x04, 0x0161);  // SGS 20190404
#endif
              }
                        
              if (verbose > 1) {
                printf(" Trigger Time: %d %d\n",t1.tv_sec,t1.tv_usec);  
                printf("End Timing Card GPS trigger\n");
              }
              rval = send_data(msgsock, &msg, sizeof(struct DriverMsg));
              break;
            ...
            case TIMING_WAIT:
              ...
              if (configured) {
                ...
                if (xfercount < max_seq_count) {
                  gettimeofday(&t0,NULL);
                  msg.status+=-2;
                  printf("Wait:  %8d %8d FIFO Underflow 0x%x Empty: 0x%x time "
                        ":: sec: %8d usec:%8d\n", xfercount,max_seq_count,
                          under_flag,empty_flag,t0.tv_sec,t0.tv_usec);
                }
