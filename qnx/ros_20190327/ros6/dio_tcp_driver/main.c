...
int32_t num_freqs[MAX_RADARS],num_beamcodes[MAX_RADARS];
int32_t num_fsteps[MAX_RADARS],foffset[MAX_RADARS];
...
*/
  ...
  if (configured) {
    ...
    if (temp==-1) {
      ...
      if (verbose > 1) fprintf(stderr, "       PLX9052 configuration "
                                        "successful!\n");
    }
  ...
  while (1) {
    ...
    if( (msgsock==-1) ) {
      ...
      while (rval>=0) {
        ...
        if (rval==0) {
          if (verbose > 0) printf("Remote Msgsock %d client disconnected "
                                  "...closing\n",msgsock);
          break;
        } 
      ...
      if ( FD_ISSET(msgsock,&rfds) && rval>0 ) {
        ...
        switch (datacode) {
          ...
          case DIO_CtrlProg_READY:
            if (verbose > 1) printf("\nAsking to set up dio info for client "
                                    "that is ready\n");  
          ...
          case DIO_GET_TX_STATUS:
            ...
            if (verbose > 0) {
              for (tx=0;tx<16;tx++) {
                if (verbose > 0 )
                  printf("  Transmitter: %d AGC: %d LOWPWR: %d\n",
...
//            break;
          ...
          case DIO_RXFE_SETTINGS:
            if (verbose > 1) printf("DIO driver: Re-configuring RXFE "
                                    "Settings\n");
          ...
          case DIO_TABLE_SETTINGS:
            if (verbose > 1) fprintf(stdout,"DIO driver: Re-configuring "
                                            "Beam Table Settings\n");
...
/* FH beam table
                ...
                for (c=0;c<num_cards[r];c++) { 
                  ...
                  final_beamcodes[r][c] =
                                malloc(sizeof(int32_t)*num_beamcodes[r]); 
                  rval = fread(final_beamcodes[r][c],sizeof(int32_t),
                                num_beamcodes[r],beamtablefile);
                  if (final_attencodes[r][c]!=NULL)
                     free(final_attencodes[r][c]);
                  final_attencodes[r][c]=
                               malloc(sizeof(int32_t)*num_beamcodes[r]); 
                  rval = fread(final_attencodes[r][c],sizeof(int32_t),
                               num_beamcodes[r],beamtablefile);
                  if (final_freqs[r][c]!=NULL) free(final_freqs[r][c]);
                  final_freqs[r][c]=malloc(sizeof(double)*num_beamcodes[r]); 
                  rval = fread(final_freqs[r][c],sizeof(int32_t),
                               num_beamcodes[r],beamtablefile);
                  if (final_angles[r][c]!=NULL) free(final_angles[r][c]);
                  final_angles[r][c]=malloc(sizeof(double)*num_beamcodes[r]); 
                  rval = fread(final_angles[r][c],sizeof(int32_t),
                               num_beamcodes[r],beamtablefile);
                }
                fprintf(stdout,"Closing: %s\n",filename);
                fclose(beamtablefile);
                ...
                angles[r]=NULL;
                for (c=0;c<num_cards[r];c++) { 
                  if (final_beamcodes[r][c]!=NULL)
                    free(final_beamcodes[r][c]);
