...
//  clock_gettime(CLOCK_REALTIME, &start);
  ...
  if (configured) {
    ...
    // x90 to address x11B produced 10 MHz reference signal output
    *((uint32_t*)(BASE1+0x118))=0x90000021;
    ...
    //set rate synthesiser rate to a default of 100ms (10pps)
    *((uint32_t*)(BASE1+0x128)) = 2;
    // x12e: event trigger on external event, falling edge 
    *((uint32_t*)(BASE1+0x12c)) = 0x02000f00;
    ...
    if (!first_gps_lock && configured) {
      ...
      if (!first_gps_lock) {
        ...
        if (wait_for_lock) {
          ...
          fprintf(stderr,"GPS Not Waiting for Lock, willing to talk to ROS "
                          "now\n");
          fflush(stderr);
          first_gps_lock=1; 
          locked=1;
        ...
        if (FD_ISSET(msgsock,&rfds) && rval > 0) {
          ...
          switch (datacode) {
            ...
            case GPS_GET_SOFT_TIME:
              if (verbose > 1)
                fprintf(stderr," GPS_GET_SOFT_TIME: Configured: %d Locked: "
                               "%d\n", configured,locked); 
              fflush(stderr);
              gpssecond=0;
              gpsnsecond=0;
            ...
            case GPS_GET_EVENT_TIME:
              ...
              if (configured && locked) {
                if (verbose > 1)
                  fprintf(stderr," Get GPS  EVENT: Configured: %d Locked: "
                                 "%d\n",configured,locked); 
                msg.status = get_event_time(&gpssecond,&gpsnsecond,BASE1);
              } else {
                if (verbose > 1)
                  fprintf(stderr," Get SOFT EVENT: Configured: %d Locked: "
                                 "%d\n",configured,locked); 
                msg.status = get_software_time(&gpssecond,&gpsnsecond,BASE1);
              }
