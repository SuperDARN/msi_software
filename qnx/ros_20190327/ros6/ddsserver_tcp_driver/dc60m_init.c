...
//    printf(" interp_val: %d\n",interp_val);  
  ...
  if (interp_val < 11)
    control_value = control_value | (uint32_t)(((uint32_t)(interp_val % 7) &
                    0x03) << 5);  //Sets special case of CIC interpolation
                                  //when interp =8,9 or 10
  
  pdebug("DC60M_INIT - MODULE_CONTROL_REG %x %x\n",
          ICS660_MODULE_CONTROL_REG,control_value);
  ics660_set_parameter(ics660, (int)ICS660_MODULE_CONTROL_REG,&control_value,
  ...
  for (chip=1; chip<=4; chip++){
    ...
    /* reset chip */
    dc60m_p.value = 0xff;
    ics660_set_parameter(ics660, (int)ICS660_DC60M_RESET,&dc60m_p,
                         sizeof(dc60m_p));

    /* set the miscellaneous register  to have OS_MODE=1*/
    dc60m_p.value = (uint32_t)0x00000080;
    ics660_set_parameter(ics660, (int)ICS660_MISCELANEOUS,&dc60m_p,
                         sizeof(dc60m_p));
    ...
    ics660_set_parameter(ics660, (int)ICS660_SYNC_MODE,&dc60m_p,
                         sizeof(dc60m_p));
  }
  ...
  for (chip=1; chip<=4; chip++){
    ...
    ics660_set_parameter(ics660, (int)ICS660_INTERPOLATION_MODE,&dc60m_p,
                         sizeof(dc60m_p));
    
    dc60m_p.value = (uint32_t) 0x00000055; // sets gain sync
    ics660_set_parameter(ics660, (int)ICS660_CHANNEL_A_SYNC,&dc60m_p,
                         sizeof(dc60m_p));
    ics660_set_parameter(ics660, (int)ICS660_CHANNEL_B_SYNC,&dc60m_p,
                         sizeof(dc60m_p));
    ics660_set_parameter(ics660, (int)ICS660_CHANNEL_C_SYNC,&dc60m_p,
                         sizeof(dc60m_p));
    ics660_set_parameter(ics660, (int)ICS660_CHANNEL_D_SYNC,&dc60m_p,
                         sizeof(dc60m_p));
    ics660_set_parameter(ics660, (int)ICS660_CHANNEL_FLUSH_MODE,&dc60m_p,
                         sizeof(dc60m_p));
...
//    printf(" chip %d SCALE: %d\n",chip,SCALE);  
    SCALE = (uint32_t)floor(4.*log((double)interp_val)/log((double) 2.)) -
                            3. - 12.*BIG_SHIFT + 1;
//    SCALE = (uint32_t) floor(4.*log((double)interp_val)/log((double) 2.)) - 3. - 12.*BIG_SHIFT ;
    if (SCALE > 15 ) SCALE=(uint32_t)15;

    cicgain = (float)pow((double)interp_val,(double)4.)*pow((double)2.,
                         (double)-(SCALE+12*BIG_SHIFT+3));
...
//    printf(" chip %d cicgain: %lf\n",chip,cicgain);  
    ...
    ics660_set_parameter(ics660, (int)ICS660_INTERPOLATION_GAIN,&dc60m_p,
                         sizeof(dc60m_p));

    /* set the CIC interpolation */
    interp0 = 0x000000ff & (uint32_t)interp_val-1;
    dc60m_p.value = interp0;
    ics660_set_parameter(ics660, (int)ICS660_INTERPOLATION_BYTE0,&dc60m_p,
                         sizeof(dc60m_p));

    interp1 = ((interp_val-1) >> 8) & 0x0000003f;
    dc60m_p.value = interp1;
    ics660_set_parameter(ics660, (int)ICS660_INTERPOLATION_BYTE1,&dc60m_p,
                         sizeof(dc60m_p));
    
    /* set counter mode to ffff */
    dc60m_p.value = (uint32_t)0xffffffff;
    ics660_set_parameter(ics660, (int)ICS660_COUNTER_BYTE0,&dc60m_p,
                         sizeof(dc60m_p));
    ics660_set_parameter(ics660, (int)ICS660_COUNTER_BYTE1,&dc60m_p,
                         sizeof(dc60m_p));    
...
//       printf ("    chan: %d   input gain: %d\n",i,pg_reg.value);
       ...
       ics660_set_parameter(ics660, (int)ICS660_SET_PAGE_REG,&pg_reg,
                            sizeof(pg_reg));
    }      
