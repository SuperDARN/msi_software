...
int msi_timing_sequence(int numclients, int max_seq_length,
                        struct ControlPRM  *clients,
                        int seqlength[MAX_RADARS][MAX_CHANNELS],
                        int *client_seqs[MAX_RADARS][MAX_CHANNELS],
...
//  short max_val[5]={0,15500,23000,32767,32767};
  ...
  if (verbose > -1)
    printf("Inside MSI timing sequence clients:%d SC: %d Max: %d\n",
  ...
  for (r=0; r<MAX_RADARS; r++) {
    ...
    for (c=0; c<MAX_CHANNELS; c++)
      if (active[r][c] == c) num_active_chans[r]++;
  } 
  ...
  for (r=0; r<MAX_RADARS; r++) {
    for (c=0; c<MAX_CHANNELS; c++) {
      if (active[r][c] == c) {
        rr = r+2;
        if (verbose > 0) {
          fprintf(stdout, "Radar: %d Active Number of channels: %d  Max Value:"
                          " %hd %hx\n", r,num_active_chans[r],
                          max_val[r][num_active_chans[r]],
                          max_val[r][num_active_chans[r]]);
          fprintf (stdout,"IF Signal: radar: %d: Output: %d :: %d %d %d %d\n",
                          r,rr,seqlength[r][c],max_seq_length,dds_nchan,
                          DDS_MAX_CHANNELS);
        }
        ...
        for (i=0; i<seqlength[r][c]; i++) {
          if (verbose > 1) {
            if (client_seqs[r][c][i] != 0 && flag == 0) {
              fprintf(stdout,"Beginning of TX: %d\n", i);
              flag = 1;
            }
            if (client_seqs[r][c][i] == 0 && flag == 1) {
              fprintf(stdout,"End of TX: %d\n", i-1);
              flag = 0;
            }
            fflush(stdout);
          }
          ...
          while (cc < DDS_MAX_CHANNELS) {
            if (active[r][cc] == c) {
              if (IMAGING == 0) {   /* NOT imaging */
                dds_chan = r*DDS_MAX_CHANNELS+cc;
                ind = (ioff + dds_chan);
                dac_value.samples.low  = (short)client_seqs[r][c][i] *
                /* same?! */                  max_val[r][num_active_chans[r]];
                dac_value.samples.high = (short)client_seqs[r][c][i] *
                                              max_val[r][num_active_chans[r]];

                if (ind < SC) data_ar[ind] = (uint32_t) dac_value.two_samp;
                else {
                  if (dac_value.samples.low !=0 || dac_value.samples.high !=0) {
                    fprintf(stderr, "Timing Sequence Error:: Index out of "
                                    "bounds\n");
                    fprintf(stderr, "  ind: %d SC: %d data_ar: %p\n",
                                    ind,SC,data_ar);
                    fprintf(stdout, "  r: %d c: %d i: %d num_active_chans[r]:"
                ...
                for (a=0; a<4; a++) {
                  ...
                  if (ind < SC) data_ar[ind] = (uint32_t) dac_value.two_samp;
                    ...
                    if (dac_value.samples.low!=0 || dac_value.samples.high!=0) {
                      fprintf(stderr, "Imaging Timing Sequence Error:: Index "
                                      "out of bounds\n");
                      fprintf(stderr,"  ind: %d SC: %d data_ar: %p\n",
                                      ind,SC,data_ar);
                      fprintf(stdout, "  r: %d c: %d i: %d num_active_chans[r]:"
        ...
        if (ifmode == 1) {
          if (IMAGING ==0 ) {
            if (verbose > 0)
              fprintf (stdout, "IF Signal radar: %d Output: %d :: "
                               "%d %d %d %d\n",r,rr,seqlength[r][c],
                                max_seq_length,dds_nchan,DDS_MAX_CHANNELS);
            for (i=0; i<max_seq_length-1; i++) {
              ...
              if (ind < SC) data_ar[ind] = (uint32_t) dac_value.two_samp;
                ...
                if (dac_value.samples.low!=0 || dac_value.samples.high!=0) {
                  fprintf(stderr, "IF Timing Sequence Error:: Index out of "
                                  "bounds\n");
                  fprintf(stderr, "  ind: %d SC: %d data_ar: %p\n",
                                  ind,SC,data_ar);
                  fprintf(stdout, "  rr: %d cc: %d i: %d num_active_chans[r]:"
  ...
  for (pci_ind=pci_min; pci_ind < DDS_MAX_CARDS; pci_ind ++) {
    ...
    ics660_set_parameter((int)ics660[pci_ind],ICS660_BANK_LENGTH,
                              &bank_length, sizeof(bank_length));
    ics660_set_parameter((int)ics660[pci_ind],ICS660_BANK_WIDTH,
                              &bank_width, sizeof(bank_width));
    ics660_set_parameter((int)ics660[pci_ind],ICS660_TRANS_LENGTH,
                              &transmit_length, sizeof(transmit_length));

    /* Write DAC Reset register */
    reset_value = RESET_VALUE;
    ics660_set_parameter((int)ics660[pci_ind],ICS660_DAC_RESET,
                              &reset_value, sizeof(RESET_VALUE));
  ...
  for (pci_ind=pci_min; pci_ind < DDS_MAX_CARDS; pci_ind ++) 
    ics660_set_parameter((int)ics660[pci_ind],ICS660_DAC_ENABLE,
                              &enable, sizeof(enable));

  for (pci_ind=pci_min; pci_ind < DDS_MAX_CARDS; pci_ind ++) 
    ics660_set_parameter((int)ics660[pci_ind],ICS660_SET_DC_READY,
                              &enable, sizeof(enable));
  
  for (pci_ind=pci_min; pci_ind < DDS_MAX_CARDS; pci_ind ++) 
    ics660_set_parameter((int)ics660[pci_ind],ICS660_RELEASE_RESETS,
                              &enable, sizeof(enable));
#endif
  free(data_ar);      
