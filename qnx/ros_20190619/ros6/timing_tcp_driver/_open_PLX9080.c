...
int _open_PLX9080(unsigned int *BASEA, unsigned int *BASEB,
                  unsigned int *BASEB_phys, int *pci_handle,
                  unsigned int *mmap_io_ptr, int *interrupt_line, int print)
{
  ...
  *pci_handle = pci_attach(flags);
  ...
  if (temp != PCI_SUCCESS) {
    perror("Cannot read from configuration space of the Timing card");
    return -1;
  }
