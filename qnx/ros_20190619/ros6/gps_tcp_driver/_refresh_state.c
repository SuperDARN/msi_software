...
/*
  ...
  while(1) {
    ...
    if (configured && (stop.tv_sec-start.tv_sec) >= rr) {
      rt = get_state();

      if (rt && pr) reduce rr, but not below REFRESHRATE
      if (rt && !pr) keep rr same
      if (!rt && !pr) increase rr, but not beyond 10 minutes
      pr = rt;
    }
    sleep for up to 1 minute
  }
 */
}
      ...
      clock_settime(CLOCK_REALTIME,&now);
        ...
        if (verbose < 0) {
          printf("  %s %ld: Update System Time: time diff  %13.9f s\n",
                  ctime(&dstat.gpssecond), dstat.gpsnsecond, timediff);
          fflush(stdout);
        }
