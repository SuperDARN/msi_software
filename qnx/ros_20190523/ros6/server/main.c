...
struct Thread_List_Item *cp_threads;
pthread_mutex_t cp_list_lock, ros_state_lock, coord_lock, exit_lock;
pthread_mutex_t clr_lock, dds_comm_lock, timing_comm_lock, gps_comm_lock;
pthread_mutex_t timing_comm_lock, recv_comm_lock, dio_comm_lock;
...
int *ready_state_pointer; // , *ready_count_pointer;    // SGS
...
char *cp_list_lock_buff=NULL;
char *exit_lock_buffer=NULL;
char *coord_lock_buffer=NULL;
...
void graceful_cleanup(int signum)
{
  ...
  if (verbose > -1)
    fprintf(stderr,"Attempting graceful clean up of control program threads\n");
  ...
  if (verbose > 0)
    fprintf(stderr,"Done with control program threads now lets do worker "
                    "threads\n");
...
*/
  ...
  pthread_mutex_destroy(&cp_list_lock);
...
/* end DIO */
  ...
  cp_list_lock_buff = realloc(NULL, 1);
  strcpy(cp_list_lock_buff, "");
 ...
 */
  ...
  pthread_mutex_init(&cp_list_lock, NULL);
  pthread_mutex_init(&coord_lock, NULL);
  pthread_mutex_init(&exit_lock, NULL);
//  pthread_mutex_init(&clr_lock, NULL);    // SGS
...
//  *ready_count_pointer=0; //no control programs ready   // SGS
  *trigger_state_pointer = 0; //pre-trigger state
  ...
  fclose(fd);
  blacklist = (struct BlackList*) malloc(sizeof(struct BlackList) *
                                  (restrict_count+Max_Control_THREADS*4));
...
*/
  ...
  rc = pthread_create(&thread, NULL, (void *)&settings_parse_ini_file,
                                     (void *)&site_settings);
  pthread_join(thread,NULL);
  rc = pthread_create(&thread, NULL, (void *)&settings_rxfe_update_rf,
                                     (void *)&site_settings.rf_settings);
  pthread_join(thread,NULL);
  rc = pthread_create(&thread, NULL, (void *)&settings_rxfe_update_if,
                                     (void *)&site_settings.if_settings);
  pthread_join(thread,NULL);
...
// SGS FH uses unixsocks here...
  if (verbose > 0)
    fprintf(stderr,"Opening DIO Socket %s %d\n",diohostip,dioport);
...
//  rc = pthread_create(&log_thread, NULL, log_handler, NULL);
  ...
  rc = pthread_create(&thread, NULL, &rx_rxfe_settings,(void *)&site_settings);
  pthread_join(thread,NULL);
 ...
 */
  ...
  while (num_threads < Max_Control_THREADS) {
    ...
    cp = control_init();
    cp->state->socket = newsockfd;
    if (verbose > 1)
      fprintf(stderr,"main: new client socket %d %d\n",
                      cp->state->socket=newsockfd,newsockfd);

    /* create a new thread to process the incomming request */
    if (verbose > 1) fprintf(stderr,"Main: Client Thread\n");
    rc = pthread_create(&thread, NULL, (void *)control_handler,(void *)cp);
    if (cp_threads == NULL) {
      ...
      cp_threads=thread_list;
    } else {
      fprintf(stderr," Existing threads\n");
      thread_list=malloc(sizeof(struct Thread_List_Item));
      thread_list->next=NULL;
      thread_list->prev=NULL;
      cp_threads->next=thread_list;
      thread_list->prev=cp_threads;
      cp_threads=thread_list;
    }
    if (verbose > 1 ) {
    ...
    /* set thread linkages */
    cp_threads->id = thread;
    cp_threads->last_seen = current_time;
    cp_threads->timeout = default_timeout;
    cp_threads->data = cp;
    thread_list = cp_threads; 
    cp->state->thread = thread_list;
    num_threads = 0;
    while (thread_list != NULL) {
