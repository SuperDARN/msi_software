...
//    ready_count++;    // SGS
  ...
  if (trigger_state < 2) { 
    ...
    while (thread_list != NULL) {
      ...
      if (verbose > 0) printf("Coord: thread\n");
      ...
      if (cprog != NULL && cprog->active == 1) {
        ...
        if (verbose > 0) printf("Coord: ncp:%d nready:%d\n", ncp,nready);
      }
  ...
  if (verbose > 0) {
    printf("Coord: ncp:%d nready:%d ready_state:%d\n",
            ncp, nready, ready_state);
    fflush(stdout);
  }
  ...
  switch (ready_state) {
    ...
    case 2:        /* all  control programs ready to trigger */
      ...
      */
      ...
      if (verbose > 0) {
        printf("Coord: pretrigger threads done\n");
        printf("Coord: sleeping for 1 ms for some reason...\n");
        fflush(stdout);
      }
      ...
      if (verbose > 0) {
        printf("Coord: done sleeping\n");
        printf("Coord: start timing_trigger thread\n");
        fflush(stdout);
      }
      ...
      if (verbose > 0) { 
        printf("Coord: done with timing_trigger.\n");
      ...
      if (cp->active == 1) { 
        ...
        if (txread[cp->parameters->radar-1]) {
          ...
          i++;    // SGS: this is missing from FH code... but no loop. WTF?!
        }
      }

      if (verbose > 0) {
        printf("Coord: finished DIO_tx_status\n");
        fflush(stdout);
      }
...
//      pthread_join(threads[i],NULL);    // SGS from FH code
      ...
      if (verbose > 0) {
        printf("Coord: finished posttrigger threads\n");
        fflush(stdout);
      }
        ...
        if (cprog != NULL) {
          if (cprog->state != NULL) {
            if (cprog->state->ready == 1) {
              ...
              if (verbose > 0) printf("Coord: processing...\n");
            }
          }
        }
        thread_list = thread_list->prev;
      ...
      if (verbose > 0) {
        printf("Coord: done with updating thread status.\n");
        fflush(stdout);
      }
