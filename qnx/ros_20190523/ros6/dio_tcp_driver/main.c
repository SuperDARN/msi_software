        ...
        if ( FD_ISSET(msgsock,&rfds) && rval>0 ) {
          ...
          switch (datacode) {
            ...
            case DIO_CtrlProg_READY:
              if (verbose > 1)
                printf("\nAsking to set up dio info for client that is "
...
//            break;
            ...
            case DIO_CLRFREQ:
              ...
              if (ifmode) set_RXFE_EEPROM_address(IOBASE, rf_addr);
              msg.status=1;
              rval=send_data(msgsock, &msg, sizeof(struct DriverMsg));
            ...
            case DIO_RXFE_RESET:
              if (verbose > 1) printf("\nDIO reset rxfe\n");  
              // Set up RXFE for each radar
              if (ifmode) set_RXFE_EEPROM_address(IOBASE, if_addr);
              else        set_RXFE_EEPROM_address(IOBASE, rf_addr);
              msg.status=1;
              rval=send_data(msgsock, &msg, sizeof(struct DriverMsg));
              break;
            ...
            case DIO_RXFE_SETTINGS:
              if (verbose > 1)
                printf("DIO driver: Re-configuring RXFE Settings\n");
            ...
            case DIO_TABLE_SETTINGS:
              if (verbose > 1)
                printf("DIO driver: Re-configuring Beam Table Settings\n");
