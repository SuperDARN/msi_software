...
*/
    ...
    if(dprm.status==0) {
      ...
      if (debug) {
        fprintf(stderr,"CVE GET_DATA: recv main\n");
      }
      TCPIPMsgRecv(sock, rdata.main, sizeof(uint32)*dprm.samples);
      if (debug) {
        fprintf(stderr,"CVE GET_DATA: recv back\n");
      }
    ...
    if (debug) {
      ...
      fprintf(stderr,"CVE Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        ...
        fprintf(stderr,"CVE seq %d :: rdata.main 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.main)[8],((int16 *)rdata.main)[9],
           ((int16 *)rdata.main)[10],((int16 *)rdata.main)[11]);
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"CVE seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"CVE seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        rngoff=2*rxchn; 
        if (debug) 
        fprintf(stderr,"CVE seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"CVE seq %d :: ACFSumPower\n",nave);
        ...
        fprintf(stderr,"CVE seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"CVE seq %d :: ACFCalculate acf\n",nave);
        ...
        fprintf(stderr,"CVE seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"CVE seq %d :: ACFCalculate xcf\n",nave);
          ACFCalculate(&tsgprm,(int16 *) dest,rngoff,skpnum!=0,
        ...
        fprintf(stderr,"CVE seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"CVE seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"CVE SiteIntegrate: iqsize in bytes: %d in 16bit samples:  %d in 32bit samples: %d\n",iqsze,iqsze/2,iqsze/4);
