/* site.c
  ...
  20190503 SGS original version ripped from site.cve.*.c  

  Common site file for CV radars.

  To make site files (cvw or cve):

  1. make a copy of this file and
  2. do a global search and replace for XXX and Xxx with appropriate radar code
  3. EDIT THE  "Site Specific Block" below, VERY IMPORTANT
...
void SiteXxxExit(int signum)
{
  ...
  switch(signum) {
    case 2:
      cancel_count++;
      XXX_exit_flag = signum;
      printf("SiteXxxExit: Sig 2: %d\n", XXX_exit_flag); 
      if (cancel_count < 3 )
        break;

    case 0:
      /*printf("SiteXxxExit: Sig 0: %d\n", XXX_exit_flag); */
      if (XXX_exit_flag != 0) {
        msg.type = QUIT;
    ...
    default:
      printf("SiteXxxExit: Sig %d: %d\n", signum, XXX_exit_flag); 
      if (XXX_exit_flag == 0) {
        XXX_exit_flag = signum;
      }
      break;
...
int SiteXxxStart()
{
  signal(SIGPIPE, SiteXxxExit);
  signal(SIGINT,  SiteXxxExit);
  signal(SIGUSR1, SiteXxxExit);
  ...
  /* Site Specific Block */
  rnum = *; // Radar number to register: 1-cve  2-cvw
  cnum = 1; // Channel number to register
  backward = *; // Beam scan direction: 0-cve 1-cvw
  //invert=1;   // Invert main array samples to account for inversion between
                //   main and intfer array; not sure this was correct: SGS
 
  /* start and end beams; almost certainly overwritten by command line args */
  sbm = *;  // start beam:  0-cve  23-cvw
  ebm = *;  // start beam: 23-cve   0-cvw
  ...
  rxchn = 1;
  if (debug)
    fprintf(stderr,"SiteXxxStart: rxchn=%d\n", rxchn);

  return 0;
}

int SiteXxxSetupRadar()
{
  ...
  sprintf(sharedmemory,"IQBuff_Xxx_%d_%d",rnum,cnum);
...
int SiteXxxStartScan()
{
  return 0;
}

int SiteXxxStartIntt(int sec,int usec)
{
  struct ROSMsg smsg,rmsg;
  int total_samples=0;
  double secs;
  ...
  SiteXxxExit(0);
  if (debug) {
    fprintf(stderr,"SiteXxxStartInt: start\n");
  }
  ...
  if (debug) fprintf(stderr,"SiteXxxStartInt: end\n");

  return 0;
}

int SiteXxxFCLR(int stfreq,int edfreq)
{
  int32 tfreq;
  struct ROSMsg smsg,rmsg;
...
int SiteXxxIntegrate(int (*lags)[2])
{
  ...
  if (debug) fprintf(stderr,"XXX SiteIntegrate: start\n");
  ...
  skpnum = tsgprm.smdelay;  // skpnum != 0  returns 1, which is used as the
                            // dflg argument in ACFCalculate to enable smdelay
                            // usage in offset calculations
...
*/
    ...
    if (time_diff > 0.0) break; // finished with beam sounding
    ...
    rdata.back = NULL;
    TCPIPMsgSend(sock,&smsg,sizeof(struct ROSMsg));
    if (debug) fprintf(stderr,"XXX GET_DATA: recv dprm\n");

    TCPIPMsgRecv(sock,&dprm,sizeof(struct DataPRM));
    if(rdata.main) free(rdata.main);
    if(rdata.back) free(rdata.back);

    if (debug) fprintf(stderr,"XXX GET_DATA: samples %d status %d\n",
                               dprm.samples,dprm.status);

    if (dprm.status == 0) {
      if (debug)
        fprintf(stderr,"XXX GET_DATA: rdata.main: uint32: %d array: %d\n",
      ...
      if (debug) fprintf(stderr,"XXX GET_DATA: recv main\n");

      TCPIPMsgRecv(sock, rdata.main, sizeof(uint32)*dprm.samples);
      if (debug) fprintf(stderr,"XXX GET_DATA: recv back\n");
      ...
      if (debug) fprintf(stderr,"XXX GET_DATA: trtimes length %d\n",
                                badtrdat.length);

      TCPIPMsgRecv(sock, &badtrdat.length, sizeof(badtrdat.length));
      if (debug) fprintf(stderr,"XXX GET_DATA: badtrdat.start_usec: "
                                "uint32: %d array: %d\n",
                                (int)sizeof(uint32),
                                (int)sizeof(uint32)*badtrdat.length);

      badtrdat.start_usec = malloc(sizeof(uint32)*badtrdat.length);
      badtrdat.duration_usec = malloc(sizeof(uint32)*badtrdat.length);
      if (debug) fprintf(stderr,"XXX GET_DATA: start_usec\n");

      TCPIPMsgRecv(sock, badtrdat.start_usec, sizeof(uint32)*badtrdat.length);
      if (debug) fprintf(stderr,"XXX GET_DATA: duration_usec\n");
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"XXX GET_DATA: type   = %c\n",rmsg.type);
      fprintf(stderr,"XXX GET_DATA: status = %d\n",rmsg.status);
    }
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"XXX GET_PARAMETERS: type   = %c\n",rmsg.type);
      fprintf(stderr,"XXX GET_PARAMETERS: status = %d\n",rmsg.status);
      fprintf(stderr,"XXX Number of samples: dprm.samples:%d tsprm.samples:%d "
                     "total_samples:%d\n",dprm.samples,tsgprm.samples,
                      total_samples);
      fprintf(stderr,"XXX nave = %d\n",nave);
    }
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if ((iqoff+total_samples*2*sizeof(uint32)) < IQBUFSIZE) {
        ...
        dest += total_samples*sizeof(uint32); // skip ahead num of samples *
                                              // 32 bit per sample to account
                                              // for rdata.main
      ...
      iqsze += total_samples*sizeof(uint32)*2;  // Total of number bytes so far
                                                // copied into samples array
      if (debug) {
        fprintf(stderr,"XXX seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"XXX seq %d :: rdata.main 16bit 8-11: "
                       "%8d %8d %8d %8d\n",nave,
        ...
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"XXX seq %d :: samples    16bit 8-11: "
                       "%8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);

        fprintf(stderr,"XXX seq %d :: rdata.back 16bit 8-11: "
                       "%8d %8d %8d %8d\n",nave,
        ...
        fprintf(stderr,"XXX seq %d :: iqsze: %8d\n",nave,iqsze);
      }

      /* calculate ACF and XCF */   
      if (mplgexs == 0) {
        dest = (void *)(samples);
        dest += iqoff;
        rngoff = 2*rxchn; 
        if (debug) {
          fprintf(stderr,"XXX seq %d :: rngoff %d rxchn %d\n",
                                  nave,rngoff,rxchn);
          fprintf(stderr,"XXX seq %d :: ACFSumPower\n",nave);
        }
        ...
        if (debug) {
          fprintf(stderr,"XXX seq %d :: rngoff %d rxchn %d\n",
                         nave,rngoff,rxchn);
          fprintf(stderr,"XXX seq %d :: ACFCalculate acf\n",nave);
        }

        ACFCalculate(&tsgprm, (int16 *)dest, rngoff, skpnum!=0, roff, ioff,
                     mplgs, lagtable, acfd, ACF_PART, 2*total_samples, badrng,
        ...
        if (xcf == 1) {
          if (debug) {
            fprintf(stderr,"XXX seq %d :: rngoff %d rxchn %d\n",
                           nave,rngoff,rxchn);
            fprintf(stderr,"XXX seq %d :: ACFCalculate xcf\n",nave);
          }
          ACFCalculate(&tsgprm, (int16 *)dest, rngoff, skpnum!=0, roff, ioff,
                       mplgs, lagtable, xcfd, XCF_PART, 2*total_samples,
        ...
        if ((nave > 0) && (seqatten[nave] != seqatten[nave])) {
          if (debug) {
            fprintf(stderr,"XXX seq %d :: rngoff %d rxchn %d\n",
                            nave,rngoff,rxchn);
            fprintf(stderr,"XXX seq %d :: ACFNormalize\n",nave);
          }

          ACFNormalize(pwr0,acfd,xcfd,tsgprm.nrang,mplgs,atstp); 
        }  

        if (debug) fprintf(stderr,"XXX seq %d :: rngoff %d rxchn %d\n",
                                    nave,rngoff,rxchn);
      ...
      fprintf(stderr,"SiteXxxIntegrate:: Bad pulse sequence status: %d\n",
                      dprm.status);
      fflush(stderr);
    }
  ...
  /* Now divide by nave to get the average pwr0 and acfd values for the 
   ...
   if (mplgexs == 0)
     ...
     if (nave > 0) ACFAverage(pwr0,acfd,xcfd,nave,tsgprm.nrang,mplgs);

   else // ACFEX calculation

    if (nave > 0) ACFexCalculate(&tsgprm, (int16 *)samples, nave*smpnum,
                                  nave, smpnum, roff, ioff, mplgs, mplgexs,
                                  lagtable, lagsum, pwr0, acfd, &noise);

   free(lagtable[0]);
   free(lagtable[1]);

   if (debug) {
     fprintf(stderr,"XXX SiteIntegrate: iqsize in bytes: %d in 16bit samples:"
                    "  %d in 32bit samples: %d\n",iqsze,iqsze/2,iqsze/4);
     fprintf(stderr,"XXX SiteIntegrate: end\n");
   }
...
int SiteXxxEndScan(int bsc,int bus)
{
  struct ROSMsg smsg,rmsg;
  struct timeval tock;
  struct timeval tick;
  ...
  while (1) {
    ...
    count++;
    usleep(50000);    // note: we are waiting for 50 ms and checking again, so
                      //       it is possible that we start the next scan up to
                      //       <50 ms LATE... Why isn't the sleep time based on
                      //       how much time is left in scan? SGS
    gettimeofday(&tick,NULL);
  }
