...
int SiteFhwStartIntt(int sec,int usec) {
  ...
  if (debug) {
    fprintf(stderr,"SiteFhwStartInt: end\n");
  }
  return 0;
...
int SiteFhwIntegrate(int (*lags)[2]) {
  ...
  uint32 uI32,uQ32;
  if (debug) {
    fprintf(stderr,"FHW SiteIntegrate: start\n");
  }
  SiteFhwExit(0);
...
*/
    ...
    TCPIPMsgSend(sock,&smsg,sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"FHW GET_DATA: recv dprm\n");
    }
    ...
    if(dprm.status==0) {
      ...
      if (debug) {
        fprintf(stderr,"FHW GET_DATA: recv main\n");
      }
      TCPIPMsgRecv(sock, rdata.main, sizeof(uint32)*dprm.samples);
      if (debug) {
        fprintf(stderr,"FHW GET_DATA: recv back\n");
      }
      ...
      if (debug) {
        fprintf(stderr,"FHW GET_DATA: start_usec\n");
      }
      TCPIPMsgRecv(sock, badtrdat.start_usec,
                 sizeof(uint32)*badtrdat.length);
      if (debug) {
        fprintf(stderr,"FHW GET_DATA: duration_usec\n");
      }
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"FHW GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"FHW GET_DATA:status=%d\n",rmsg.status);
    }
    smsg.type=GET_PARAMETERS;
    TCPIPMsgSend(sock, &smsg, sizeof(struct ROSMsg));
    TCPIPMsgRecv(sock, &rprm, sizeof(struct ControlPRM));
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"FHW GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"FHW GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"FHW Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"FHW nave=%d\n",nave);
      fprintf(stderr,"FHW dprm.status=%d\n",dprm.status);
    }
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        fprintf(stderr,"FHW seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"FHW seq %d :: rdata.main 16bit :\n",nave);
        ...
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"FHW seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.back)[8],((int16 *)rdata.back)[9],
           ((int16 *)rdata.back)[10],((int16 *)rdata.back)[11]);
        dest += dprm.samples*4;
        fprintf(stderr,"FHW seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"FHW SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",(long int)iqsze,(long int)iqsze/2,(long int)iqsze/4);
     fprintf(stderr,"FHW SiteIntegrate: end: nave: %d\n",nave);
   }
